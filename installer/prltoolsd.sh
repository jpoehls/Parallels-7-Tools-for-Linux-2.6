...
# Don't load prl_freeze on system with 2.4 kernel
if [ $(echo $(uname -r) | cut -d'.' -f2) -ne 4 ]; then
	LOAD_PRL_FREEZE="yes"
	LOAD_PRL_PV="yes"
fi
...
start() {
    ...
    if [ -n "$LOAD_PRL_PV" ]; then
        prl_begin_msg $"Loading Parallels Paravirt driver: "
        if modprobe prl_pv; then
            prl_end_msg $"done"
        else
            prl_end_msg $"failed"
        fi
